...
// }
// if (($this->session->has_userdata('student_submit_thesis')) AND (!$this->session->userdata('student_submit_thesis'))) {
// 	unset($top_bar['student_thesis']);
// }
?>
<!DOCTYPE html>
<html lang="en">
	...
	<body class="app header-fixed sidebar-fixed aside-menu-fixed sidebar-lg-show">
			...
			?>
				<li class="nav-item dropdown">
					...
					<div class="dropdown-menu dropdown-menu-right">
						...
						<a class="dropdown-item" href="<?=site_url('file_manager/view_doc/academic_calendar_2022')?>" target="_blank">
							<i class="fas fa-calendar"></i> Academic Calendar 2022
						</a>
				...
				if ((intval(date('d')) <= 10) OR (intval(date('d')) > 28)) {
					...
					?>
						Swal.fire({
							...
							icon: 'info',
							html:
								"<p><strong>For students who still have outstanding payments by June 10, 2023, access to wifi and student's portal will be temporarily disabled.<br>and later they will not be able to take part in the final exam which will be held on June 26 2023.</strong></p>" +
								'<p><strong>For students who are working on a thesis, if all administration is not completed, they are not allowed to take part in the thesis defense in August 2023</strong></p>',
		...
		<?= modules::run('validation_requirement/vaccine/form_question');?>
		<?php
			// if ($this->session->userdata('user') == '60d0df0f-8cdd-4c5c-9cf5-8c631a39bf1e') {
				echo modules::run('apps/kpu/show_modal_voting');
			// }
			// 
			// if ($this->session->userdata('user') == '9adf484b-bdb4-40b7-8780-38a73e217861') {
...
?>
		<script>
			// if (confirm('Have you found a job placement (already working)?')) {
			...
			Swal.fire({
				...
				// title: 'Have you found a job placement (already working) or university to continue your study?',
				icon: 'warning',
				showCancelButton: true,
...
<?php
	...
	if (($this->session->has_userdata('message_academic')) AND ($this->session->userdata('message_academic'))) {
		// $this->session->set_userdata('message_academic');
?>
