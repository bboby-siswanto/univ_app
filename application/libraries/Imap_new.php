...
class Imap_new {
    ...
    public function __construct($params) {
	    ...
	    $encryption = (array_key_exists('encryption', $params)) ? $params['encryption'] : false;
    ...
    protected function formatMessage($id, $withbody=true){
        ...
        $email = array(
            ...
            'email'    => implode('@', [$header->from[0]->mailbox, $header->from[0]->host]),
            'date'     => $header->date,
            'subject'  => $subject,
            'uid'      => $uid,
		...
		// return $mailStruct;
		$attachment_data = $this->getAttachments($this->imap, $id, $mailStruct, "");
        $attachments = $this->attachments2name($attachment_data);
        ...
        if (isset($part->disposition)) {
            if (strtolower($part->disposition) == "attachment" || strtolower($part->disposition) == "ooinline") {
                ...
                $attachmentDetails = array(
                    "name"    => ($part->ifparameters) ? $part->parameters[0]->value : '',
                    "partNum" => $partNum,
