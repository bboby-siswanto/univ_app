...
class Old_api extends App_core
{
	...
	// public function check_billing_2()
	// {
	// 	$mba_unpaid_invoice = $this->Im->get_unpaid_invoice_full(false, ['02', '05', '09', '18']);
	// 	// invoice for tuition fee, semester leave, graduation fee, german language
	// 	$a_report_data = [];
	// 	if ($mba_unpaid_invoice) {
	// 		foreach ($mba_unpaid_invoice as $o_invoice) {
	// 			$send_reminder = $this->check_invoice_new($o_invoice);
	// 			if ($send_reminder) {
	// 				// $this->send_reminder($o_invoice);
...
TEXT;
	...
	public function handle_tuition_fee($mbo_invoice_detail, $a_paid_billing)
	{
		...
		$s_amount = "Rp. ".number_format($d_payment_amount, 0, ',', '.').",-";
		$s_payment_type_code = substr($mbo_invoice_detail->virtual_account, 4, 2);
		$i_installment = intval(substr($mbo_invoice_detail->virtual_account, 8, 1));
		$s_semester = intval(substr($mbo_invoice_detail->virtual_account, 6, 2));
		...
		// }else{
			if ($s_payment_type_code == '10') {
				...
				if ($i_installment > 0) {
					$s_fee_desc = 'semester '.$semester.' installment '.$i_installment;
				}
				else {
					$s_fee_desc = 'semester '.$semester;
				}

				if ($mbo_invoice_detail->virtual_account == '8310020601200204') {
					$b_is_special = true;
				}
					...
					if($mbo_invoice_detail){
						...
						if(($mbo_invoice_detail->sub_invoice_details_amount_paid != $a_parsed_data['payment_amount']) OR (is_null($mbo_invoice_detail->sub_invoice_details_datetime_paid_off))){
							switch($s_payment_type_code)
							{
								...
								case "50":
									$this->handle_open_payment_research($mbo_invoice_detail, $a_parsed_data);
									break;

								case "21":
									$this->handle_srh_payment($mbo_invoice_detail, $a_parsed_data);
									break;
	...
	public function bni_payment_notification_testing()
	{
	...
	public function bni_payment_notification()
	{
		...
		if(array_key_exists($a_data_json['client_id'], $this->a_client_ids)){
			...
			if($mbo_invoice_detail){
				$s_payment_type_code = substr($mbo_invoice_detail->virtual_account, 4, 2);
				if ($s_payment_type_code == '88') {
					$mbo_invoice_detail = $this->Im->get_invoice_detail_open_payment($a_parsed_data['trx_id'], $a_parsed_data['payment_amount']);
				}
				else {
					$this->Bm->update_billing_portal($a_parsed_data['trx_id'], $a_parsed_data);
	...
	*/
	...
	public function force_from_bni()
	{
		$s_parsed_data = '{"trx_id":"1744154645","virtual_account":"8310211135120802","customer_name":"Cindy Angrainy Mutiara Uray","trx_amount":"4125000","payment_amount":"4125000","cumulative_payment_amount":"4125000","payment_ntb":"182067","datetime_payment":"2022-10-10 21:27:33","datetime_payment_iso8601":"2022-10-10T21:27:33+07:00"}';
		$a_bni_failover = json_decode($s_parsed_data);
		$a_parsed_data = (array) $a_bni_failover;
		// print('Closed!');exit;
		...
		if($mbo_invoice_detail){
			...
			if($mbo_invoice_detail){
				...
				print('<pre>');var_dump("Invoice not found!!!");exit;
			}
		}
		else{
			$this->send_error_notification(json_encode($a_parsed_data));
