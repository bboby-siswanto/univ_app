            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="">Create Type</label><br>
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            <label class="btn btn-primary active">
                                <input type="radio" name="create_type" id="invoice_single" value="single" checked> Single
                            </label>
                            <label class="btn btn-primary">
                                <input type="radio" name="create_type" id="invoice_bulk" value="bulk"> Bulk
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-sm-12 d-none" id="amount_fined">
                    <div class="form-group">
                        <label for="fee_amount_fined">Amount Fined</label>
                        <input type="text" name="fee_amount_fined" id="fee_amount_fined" class="form-control">
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group">
...
<script>
	$(function(){
        $('#fee_amount_fined').number( true, 0 );
    ...
    ?>
        ...
        $('select#payment_type_code').on('change', function() {
            ...
            if ($('select#payment_type_code').val() == '19') {
                $('#amount_fined').removeClass('d-none');
            }
            else {
                $('#amount_fined').addClass('d-none');
            }
        ...
        function set_input_credit() {
            if (parseInt($('select#semester_id option:selected').text()) > 8) {
				if ($('#payment_type_code').val() == '02') {
                    ...
                    $('#semester_invoice').removeClass('col-md-12');
                    $('#semester_invoice').addClass('col-md-6');
                    // console.log($('#student_name').select2('data'));
                    var s_data = $('#student_name').select2('data');
                    var s_id = s_data[0].student_key;
                    // console.log(s_id);

                    $.post('<?=base_url()?>academic/score/get_credit_student_semester', {student_id: s_id, semester_id: $('select#semester_id').val()}, function(result) {
                        $('#semester_credit').val(result.sks);
                    }, 'json').fail(function(params) {
                        toastr.error('Error retrieve credit data!', 'Error');
                    })
				}
