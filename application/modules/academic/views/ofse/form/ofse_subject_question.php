...
<?php
    ...
    for ($i=1; $i <= $count_student; $i++) { 
        $data_exists = $this->General->get_where('dt_ofse_subject_question', [
            'ofse_period_id' => $ofse_period_id,
            'subject_id' => $subject_id,
            'ofse_question_sequence' => $i
        ]);
?>
        <div class="col-sm-6 mb-1">
            <div class="border rounded p-2">
                <label>Varian <?=$i;?> <i class="fas fa-<?=($data_exists) ? 'check' : '';?>"></i></label>
                <table class="table">
                    <tr>
                    ...
                    <tr>
                        <td width="100px">Number of Question</td>
                        <td>
                            <input type="number" name="question_number[]" class="form-control v_value">
                        </td>
                    </tr>
                </table>
            </div>
