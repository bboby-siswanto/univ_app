        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                ?>
                    ...
                    <!-- <input type="text" class="form-control" name="unit_time_end" id="unit_time_end" value="" readonly> -->
                    <select name="unit_time_end" id="unit_time_end" class="form-control">
                        <option value="">Please Select...</option>
                <?php
                if ($a_times) {
                    foreach ($a_times as $time) {
                ?>
                        <option value="<?=$time?>" <?= (($o_subject_delivered_data) AND ($time == (date('H:i', strtotime($o_subject_delivered_data->subject_delivered_time_end))))) ? 'selected' : '' ?>><?=$time?></option>
                <?php
                    }
                }
                ?>
...
<script>
    $(function() {
        let credit_of_subject = parseInt('<?=$class_data->curriculum_subject_credit;?>');
        ...
        $("#unit_time").on('change', function(e) {
            e.preventDefault();

            var valueselected = $('#unit_time').val();
            valueselected = valueselected.split(':');
            var hour = parseInt(valueselected[0]);
            hour += credit_of_subject;
            
            let timeend = hour + ":00";
            $('#unit_time_end').val(timeend);
        });
    });
</script>