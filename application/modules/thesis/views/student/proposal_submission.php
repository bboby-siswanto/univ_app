    ...
    <div class="col-12 mb-3">
        <div class="btn-toolbar float-right" role="toolbar" aria-label="Action button groups">
            <div class="btn-group" role="group" aria-label="First group">
                ...
                <button type="button" class="btn btn-success" id="submit_thesis_submission" data-toggle="tooltip" data-placement="bottom" title="Submit to Dean">
...
<div class="card">
    <div class="card-header">
        <strong>Thesis Proposal Submission</strong>
    </div>
    <div class="card-body">
        <form method="post" url="<?=base_url()?>thesis/submit_student_proposal" onsubmit="return false" id="form_thesis_submission">
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="border rounded p-2 m-2">
                        <div class="form-group">
                            <label for="advisor_1_update" class="required_text">Proposed Advisor</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="advisor_1_institute" id="advisor_1_institute_update" class="form-control" value="<?=$advisor_proposed_1 ? ($advisor_proposed_1->institution_name) : '';?>" disabled>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="border rounded p-2 m-2">
                        <div class="form-group">
                            <label for="thesis_title">Proposed Co-Advisor</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="advisor_2_institute" id="advisor_2_institute_update" class="form-control" value="<?=$advisor_proposed_2 ? ($advisor_proposed_2->institution_name) : '';?>" disabled>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form url="<?=base_url()?>thesis/submit_document_thesis_proposal" id="form_submit_file" onsubmit="return false">
                    <div class="row">
                        <div class="col-12">
                            <div class="border rounded p-2 m-2">
                                <div class="form-group">
                                    <label class="required_text">Thesis Proposal</label>
                                    <input type="file" name="file_proposal" id="file_proposal" class="form-control">
                                </div>
                                <small class="text-danger">File extention allowed .pdf</small>
                            </div>
                        </div>
                    </div>
                </form>
    ...
    columns: [
        {data: 'filename'},
        {
            ...
            render: function(data, type, row) {
                ...
                html += '<a href="<?=base_url()?>thesis/view_file/' + data + '" target="_blank" class="btn btn-info btn-sm" title="View Document"><i class="fas fa-eye"></i></a>';
...
$(function() {
    ...
    $('button#submit_thesis_submission').on('click', function(e) {
        ...
        if (confirm('Please re-check the contents and files that have been uploaded. Submissions cannot be changed if there is action from the deans or head of study program. Are you sure to submit?')) {
            $.blockUI();
            ...
            $.post(url, data, function(result) {
                ...
                if (result.code == 0) {
                    ...
                    setInterval(function () {
                        // location.reload();
                        window.location.href = '<?=base_url()?>thesis/student_thesis/proposal_submission'
                    }, 1000); 
                }
                else {
...
?>
    var newOption = new Option('<?=$advisor_proposed_1->advisor_name;?>', '<?=$advisor_proposed_1->advisor_id;?>', false, false);
    $('#advisor_1_update').append(newOption);
    $('#advisor_1_update').val('<?=$advisor_proposed_1->advisor_id;?>').trigger('change');
<?php
}
if ($advisor_proposed_2) {
?>
    var newOption = new Option('<?=$advisor_proposed_2->advisor_name;?>', '<?=$advisor_proposed_2->advisor_id;?>', false, false);
    $('#advisor_2_update').append(newOption);
    $('#advisor_2_update').val('<?=$advisor_proposed_2->advisor_id;?>').trigger('change');
<?php
}
?>
